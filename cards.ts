export interface ICards {
    id?: string; //adding to 'const cards' below
    dataFilter: string;
    imageSource: string;
    link: string;
    likes: number;
...
//add ID's 
export const cards: ICards[] = cardsInitial.map((item, index) => ({
    ...item,            
    id: 'fox' + Number(index + 1) as string  
}))

export const cardContent = (arrayItem: ICards): string => {
    return `
